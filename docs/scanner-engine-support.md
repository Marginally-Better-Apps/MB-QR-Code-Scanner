...
Both engines emit the same `ScannerObservation` contract: QR payload string, preview-normalized bounds in `0...1` preview coordinates, timestamp, and engine identity. Both request QR symbology only and can report every visible QR code in a frame.
